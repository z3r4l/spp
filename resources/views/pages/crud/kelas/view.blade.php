                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <div class="card-title">Data Master Kelas</div>
                        </div>
                        <div class="card-body table-responsive">
                            <a href="{{ route('create-data-kelas') }}">
                            ...
                            <table id="example" class="table table-striped mt-3" style="width:100%">
                                ...
                                <tbody>
                                    @if ($kelass)
                                        @foreach ($kelass as $index => $item)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td>{{ $item['nama_kelas'] }}</td>
                                                <td>{{ $item['rombel'] }}</td>
                                                <td>
                                                    <a href="{{ route('edit-kelas', ['id' => $item['id_kelas']]) }}"
                                                    ...
                                                    <form action="{{ route('harddelete-data-kelas', $item['id_kelas']) }}"
